...
const employeesBonus = (arr) => {
    // write your code here
    function addToSalary(sal , bonus){
        let convertSalary = parseInt(sal);
        let newSalary = convertSalary + bonus ;
        return `${newSalary}$`
    }

    for (let i = 0; i < arr.length; i++) {
        if(arr[i].workHours > 8){
            arr[i].salary = addToSalary(arr[i].salary , 100) 
        }
        else{
            arr[i].salary = addToSalary(arr[i].salary , 50)
        }
    }
   return arr ;
}
