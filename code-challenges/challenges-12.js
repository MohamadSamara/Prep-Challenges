...
const cvsFiltration = (arr) => {
    // write your code here
     return arr.filter((elem) => elem.yearsOfExperience > 4 && elem.tech === "JS").map((elem) => {
      const fullName = elem.firstName + (elem.LastName ? " " + elem.LastName : "");
      return{
        fullName,
        tech: elem.tech,
      };
...
const vowelsFiltration = (arr) => {
  ...
  return arr.filter((elem) =>
    vArr.every((vArr) => !elem.includes(vArr))
  );
